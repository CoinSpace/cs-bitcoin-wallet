...
export default class Vsize {
  ...
  static getInputSize(type, compressed = true) {
    if (type === symbols.ADDRESS_TYPE_P2PKH && compressed) return 148; // (32 + 4) + 1 + (1 + 72 + 1 + 33) + 4 = 148
    if (type === symbols.ADDRESS_TYPE_P2PKH) return 180; // (32 + 4) + 1 + (1 + 72 + 1 + 65) + 4 = 180
    if (type === symbols.ADDRESS_TYPE_P2SH) return 91; // (((32 + 4) + 1 + 23 + 4) * 4 + (1 + 1 + 72 + 1 + 33)) / 4 = 91
    if (type === symbols.ADDRESS_TYPE_P2WPKH) return 68; // (((32 + 4) + 1 + 4) * 4 + (1 + 1 + 72 + 1 + 33)) / 4 = 68
  }

  static getOutputSize(type) {
    if (type === symbols.ADDRESS_TYPE_P2PKH) return 34;
    if (type === symbols.ADDRESS_TYPE_P2SH) return 32;
    if (type === symbols.ADDRESS_TYPE_P2WPKH) return 31;
    if (type === symbols.ADDRESS_TYPE_P2WSH) return 43;
  }
  ...
  value() {
    ...
    this.#inputs.forEach((count, type) => {
      ...
      if (type === symbols.ADDRESS_TYPE_P2SH || type === symbols.ADDRESS_TYPE_P2WPKH) {
        witnessCount += count;
      }
    });
