...
import feeRates from './feeRates.js';
...
export default class BitcoinWallet extends CsWallet {
  ...
  constructor(options = {}) {
    ...
    this.#network = networks[options.development ? 'regtest' : 'mainnet'][options.crypto.platform];
  ...
  async #initPlatform() {
    if (this.crypto.platform === 'bitcoin-cash') {
      this.#bchaddr = await import('bchaddrjs');
    }
  }
  ...
  async _estimateTransactionFee({ address, value, feeRate }) {
    ...
    const { fee } = await txBuilder.build({ feeRate, address, value, changeAddress: this.#getChangeAddress() });
    return fee;
  }
  ...
  async createTransaction({ feeRate, address, amount }, seed) {
    ...
    await txBuilder.build({ feeRate, address, value, changeAddress: this.#getChangeAddress() });
    const privateAccounts = this.#getPrivateAccounts(seed);
  ...
  async estimateImport({ privateKey: wif, feeRate }) {
    ...
    const { value, fee } = await this.#prepareImport({ wif, feeRate });
    return {
  ...
  async createImport({ privateKey: wif, feeRate }) {
    ...
    await txBuilder.build({ feeRate, address: this.address, value: sendable, changeAddress: this.address });
  ...
  async loadTransactions({ cursor } = {}) {
    ...
    const transactions = txTransformer.transformTxs(txs);
  ...
  getAddressType(address) {
    try {
      ...
      if (type === 'pkh') return addressTypes.P2PKH;
      if (type === 'sh') return addressTypes.P2SH;
      if (type === 'wpkh') return addressTypes.P2WPKH;
      if (type === 'wsh') return addressTypes.P2WSH;
  ...
  #getAddressTypePath(type) {
    if (type === addressTypes.P2PKH) return this.settings.bip44;
    if (type === addressTypes.P2SH) return this.settings.bip49;
    if (type === addressTypes.P2WPKH) return this.settings.bip84;
  }
  ...
  async #prepareImport({ wif, feeRate }) {
    ...
    this.#network.addressTypes.forEach((type) => {
      ...
      if (!compressed && type !== addressTypes.P2PKH) return;
    ...
    if (value < this.#network.dustThreshold) {
      throw new errors.SmallAmountError(this.#network.dustThreshold, this.crypto.decimals);
    }
  ...
  async _loadFeeRates() {
    ...
    this.#feeRates.clear();
    fees.forEach((fee) => {
      if (fee.name === 'minimum') this.#feeRates.set(feeRates.MINIMUM, BigInt(fee.value));
      if (fee.name === 'default') this.#feeRates.set(feeRates.DEFAULT, BigInt(fee.value));
      if (fee.name === 'fastest') this.#feeRates.set(feeRates.FASTEST, BigInt(fee.value));
    });
  }
}
